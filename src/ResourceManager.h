...
public:
	...
	inline Texture* getTexture(std::string name) { return m_texMap[name]; }
	...
	void getTextureNames(std::vector<const char*>& vec) 
	{
		for (auto& texture : m_texMap)
		{
			vec.push_back(texture.first.c_str());
		}
	}
